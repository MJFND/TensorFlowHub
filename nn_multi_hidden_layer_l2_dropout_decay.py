...
def Neural_Network(x, weights, biases):
    ...
    out_layer = tf.matmul(hidden_layer_3, weights['out']) + biases['out']
    return out_layer
...
with graph.as_default():
  ...
  n_hidden_2 = 300 # 1st layer number of features
  n_hidden_3 = 50 # 1st layer number of features
  ...
  loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=tf_train_labels)+ 0.03 * tf.nn.l2_loss(weights['h1']) + 0.03 * tf.nn.l2_loss(weights['h2'])  + 0.03 * tf.nn.l2_loss(weights['h3']) + 0.03 * tf.nn.l2_loss(weights['out']))
